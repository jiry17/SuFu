...
-- output_type: Int
-- param_list: res xs t x global0 h
data Expr0_0
  = Param30_0
  | Param40_0
  | Param50_0
...
-- output_type: Int
-- param_list: hcost t xs res x h tmp2 global0
data Expr1_0
  = Param01_0
  | Param41_0
  | Param51_0
...
-- output_type: Int
-- param_list: hcost t xs res new_plan h x global0
data Expr2_0
  = Param02_0
  | Param42_0
  | Param52_0
...
instance GenSym (Int) Expr0_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam30_0] ++ [mrgParam40_0] ++ [mrgParam50_0] ++ [mrgCzero0_0]
...
instance GenSym (Int) Expr1_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam01_0] ++ [mrgParam41_0] ++ [mrgParam51_0] ++ [mrgParam71_0] ++ [mrgCzero1_0]
...
instance GenSym (Int) Expr2_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam02_0] ++ [mrgParam42_0] ++ [mrgParam52_0] ++ [mrgParam62_0] ++ [mrgParam72_0] ++ [mrgCzero2_0]
...
instance GenSym (Int) Expr4_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam04_0] ++ [mrgParam34_0] ++ [mrgCzero4_0]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        e1_0 <- (gen1 (gendepth - 1))
        res <- chooseUnionFresh (genSingle0 ++ [mrgCadd4_0 e0_0 e0_1] ++ [mrgCsub4_0 e0_2 e0_3] ++ [mrgCIte4_0 e1_0 e0_4 e0_5] ++ [mrgMin4_0 e0_6 e0_7] ++ [mrgCmul4_0 e0_8 e0_9])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
...
eval0_0 env (Param30_0) = evalVar0 env "x"
eval0_0 env (Param40_0) = evalVar0 env "global0"
eval0_0 env (Param50_0) = evalVar0 env "h"
...
eval1_0 env (Param01_0) = evalVar0 env "hcost"
eval1_0 env (Param41_0) = evalVar0 env "x"
eval1_0 env (Param51_0) = evalVar0 env "h"
eval1_0 env (Param71_0) = evalVar0 env "global0"
...
eval2_0 env (Param02_0) = evalVar0 env "hcost"
eval2_0 env (Param42_0) = evalVar0 env "new_plan"
eval2_0 env (Param52_0) = evalVar0 env "h"
eval2_0 env (Param62_0) = evalVar0 env "x"
eval2_0 env (Param72_0) = evalVar0 env "global0"
...
eval4_0 env (Param04_0) = evalVar0 env "global0"
eval4_0 env (Param34_0) = evalVar0 env "h"
...
{-
Hole grammar for #0
res xs t x global0 h 

Hole grammar for #1
hcost t xs res x h tmp2 global0 

Hole grammar for #2
hcost t xs res new_plan h x global0 
...
data LineList
  ...
  | Lcons List LineList
  deriving stock (Generic, Show, Eq)
  deriving (Mergeable, EvaluateSym, ToCon LineList, ExtractSymbolics)
    via (Default LineList)
              ...
              hcost =
                (let
                  tmp1 =
                    (head_size global0)
                in
                evalU0_0 (RefEnv [("x", (Env0 x)), ("global0", (Env0 global0)), ("h", (Env0 h))]) ((genSym (4::Int) "hole0") :: (UnionM Expr0_0)))
            in
            let
              new_plan =
                (let
                  tmp2 =
                    (Cons x (Nil Unit))
                in
                evalU1_0 (RefEnv [("hcost", (Env0 hcost)), ("x", (Env0 x)), ("h", (Env0 h)), ("global0", (Env0 global0))]) ((genSym (4::Int) "hole1") :: (UnionM Expr1_0)))
            in
            mrgIte ((x +  hcost) <=~  global0)
              (let
                insert_plan =
                  (evalU2_0 (RefEnv [("hcost", (Env0 hcost)), ("new_plan", (Env0 new_plan)), ("h", (Env0 h)), ("x", (Env0 x)), ("global0", (Env0 global0))]) ((genSym (4::Int) "hole2") :: (UnionM Expr2_0)))
              in
      ...
      case xs of
        Nil _ -> Pcons (let
            tmp3 =
              (Lnil Unit)
          in
          evalU3_0 (RefEnv [("global0", (Env0 global0))]) ((genSym (4::Int) "hole3") :: (UnionM Expr3_0))) (Pnil Unit)
        Cons h t -> extend global0 h (f t)
  ...
  let
    f xs = 
      case xs of
        Lnil _ -> 0
        Lcons h (Lnil _) -> 0
        Lcons h t -> (line_cost global0 h) +  (f t)
      ...
      case xs of
        ...
        Pcons h t -> min' (let
            tmp4 =
              (eval global0)
          in
          evalU4_0 (RefEnv [("global0", (Env0 global0)), ("h", (Env0 h))]) ((genSym (4::Int) "hole4") :: (UnionM Expr4_0))) (f t)
      
  in
  f 

main' global0 xs = 
  get_best global0 (generate global0 xs)
...
main = do
    let pairs = [
                ((((0)), ((Nil Unit))), (0))
                , ((((4)), ((Cons ((1)) ((Cons ((3)) ((Cons ((0)) ((Cons ((4)) ((Cons ((5)) ((Nil Unit))))))))))))), (1000))
                , ((((3)), ((Nil Unit))), (0))
                , ((((3)), ((Cons ((5)) ((Cons ((4)) ((Cons ((0)) ((Cons ((1)) ((Cons ((3)) ((Nil Unit))))))))))))), (1000))
                , ((((0)), ((Cons ((5)) ((Nil Unit))))), (1000))
                , ((((5)), ((Cons ((5)) ((Nil Unit))))), (0))
                , ((((3)), ((Cons ((1)) ((Cons ((5)) ((Cons ((4)) ((Cons ((5)) ((Cons ((0)) ((Cons ((3)) ((Cons ((0)) ((Cons ((4)) ((Nil Unit))))))))))))))))))), (1000))
                , ((((1)), ((Cons ((5)) ((Nil Unit))))), (1000))
                , ((((4)), ((Cons ((0)) ((Nil Unit))))), (0))
                , ((((4)), ((Cons ((1)) ((Cons ((1)) ((Cons ((3)) ((Cons ((1)) ((Cons ((2)) ((Cons ((3)) ((Cons ((4)) ((Cons ((4)) ((Nil Unit))))))))))))))))))), (3))
                , ((((0)), ((Cons ((4)) ((Cons ((0)) ((Nil Unit))))))), (1000))
                , ((((1)), ((Cons ((1)) ((Nil Unit))))), (0))
                , ((((5)), ((Cons ((1)) ((Cons ((0)) ((Cons ((2)) ((Cons ((2)) ((Nil Unit))))))))))), (0))
                , ((((1)), ((Cons ((5)) ((Nil Unit))))), (1000))
                , ((((1)), ((Nil Unit))), (0))
                , ((((4)), ((Nil Unit))), (0))
                , ((((2)), ((Cons ((0)) ((Cons ((5)) ((Cons ((1)) ((Cons ((3)) ((Cons ((5)) ((Cons ((3)) ((Cons ((5)) ((Cons ((1)) ((Cons ((1)) ((Nil Unit))))))))))))))))))))), (1000))
                , ((((0)), ((Cons ((2)) ((Cons ((1)) ((Cons ((0)) ((Nil Unit))))))))), (1000))
                , ((((4)), ((Cons ((3)) ((Cons ((5)) ((Cons ((2)) ((Cons ((2)) ((Cons ((4)) ((Cons ((4)) ((Cons ((5)) ((Cons ((4)) ((Cons ((1)) ((Nil Unit))))))))))))))))))))), (1000))
                , ((((4)), ((Cons ((4)) ((Nil Unit))))), (0))
                ]
