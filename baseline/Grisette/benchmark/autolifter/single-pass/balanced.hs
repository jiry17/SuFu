  ...
  fresh gendepth = gen0 gendepth 
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        res <- chooseUnionFresh (genSingle0 ++ [mrgAccess02_0 e1_0] ++ [mrgCeq2_0 e2_0 e2_1] ++ [mrgCless2_0 e2_2 e2_3] ++ [mrgCleq2_0 e2_4 e2_5] ++ [mrgCand2_0 e0_0 e0_1] ++ [mrgCor2_0 e0_2 e0_3] ++ [mrgCnot2_0 e0_4])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
    ...
    gen2 gendepth
      ...
      | otherwise = do
        ...
        e2_4 <- (gen2 (gendepth - 1))
        e2_5 <- (gen2 (gendepth - 1))
        res <- chooseUnionFresh (genSingle2 ++ [mrgCadd2_2 e2_0 e2_1] ++ [mrgCsub2_2 e2_2 e2_3] ++ [mrgCIte2_2 e0_0 e2_4 e2_5] ++ [mrgAccess12_2 e1_0] ++ [mrgAccess22_2 e1_1])
        return res

getFirst :: (SymBool, SymInteger, SymInteger) -> SymBool
getFirst (a, _, _) = a

getSecond :: (SymBool, SymInteger, SymInteger) -> SymInteger
getSecond (_, b, _) = b

getThird :: (SymBool, SymInteger, SymInteger) -> SymInteger
getThird (_, _, c) = c
...
eval0_0 env (False0_0) = (toSym False, 0, 0)
...
eval0_2 env (Access00_2 p0) = getFirst (evalU0_0 env p0)
...
eval1_2 env (Access01_2 p0) = getFirst (evalU1_0 env p0)
...
eval2_2 env (Access22_2 p0) = getThird (evalU2_1 env p0)
...
balanced = 
    let f cnt xs = 
                case xs of
                    ...
                    Cons h t -> 
                        let cnt = (cnt +  h) in 
                            mrgIte (cnt <~  0) ((toSym False)) (f cnt t)
                in f 0

main' = 
    single_pass balanced
