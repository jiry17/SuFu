  ...
  fresh gendepth = gen0 gendepth 
    ...
    gen1 gendepth
      ...
      | otherwise = do
        ...
        e1_5 <- (gen1 (gendepth - 1))
        e2_0 <- (gen2 (gendepth - 1))
        res <- chooseUnionFresh (genSingle1 ++ [mrgCIte1_1 e2_0 e1_4 e1_5] ++ [mrgAccess01_1 e0_0] ++ [mrgAccess11_1 e0_1] ++ [mrgAccess21_1 e0_2])
        return res
    gen2 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle2
      | otherwise = do
...
trdmin xs = 
    (getFirst (let f xs = 
                    case xs of
                        ...
                        Cons h t -> 
                            let res = (f t) in 
                                ((min' (getFirst res) (max' (getSecond res) h)), (min' (getSecond res) (max' (getThird res) h)), (min' (getThird res) h))
                    in f  xs))
...
main = do
    let pairs = [
                ...
                , ((Nil Unit), 100)
                , ((Cons (3) (Cons (-5) (Cons (2) (Nil Unit)))), 3)
