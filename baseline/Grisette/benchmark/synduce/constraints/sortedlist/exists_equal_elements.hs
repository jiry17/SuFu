...
instance GenSym (Int) Expr5_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam55_0]
    genSingle1 = [mrgParam05_1] ++ [mrgParam35_1] ++ [mrgCzero5_1]
    gen0 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle0
      | otherwise = do
        e0_0 <- (gen0 (gendepth - 1))
...
instance GenSym (Int) Expr7_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam17_0]
    genSingle1 = [mrgCzero7_1]
    gen0 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle0
      | otherwise = do
        e0_0 <- (gen0 (gendepth - 1))
...
eval5_0 env (Param55_0) = evalVar1 env "res"
...
eval5_1 env (Param05_1) = evalVar0 env "h1"
eval5_1 env (Param35_1) = evalVar0 env "h2"
...
eval7_0 env (Param17_0) = evalVar1 env "tmp1"
        ...
        ((Cons h1 t1), (Cons h2 t2)) -> mrgIte (h1 >~  h2)
            (let
              res =
                (f (t1, (snd p)))
            in
            evalU5_0 (RefEnv [("res", (Env1 res)), ("h1", (Env0 h1)), ("h2", (Env0 h2))]) ((genSym (1::Int) "hole5") :: (UnionM Expr5_0)))
            (let
              res =
                (f ((fst p), t2))
            in
  ...
  mrgIte (is_sorted_pair p)
    (let
      tmp1 =
        (target p)
    in
    evalU7_0 (RefEnv [("tmp1", (Env1 tmp1))]) ((genSym (1::Int) "hole7") :: (UnionM Expr7_0)))
    ((toSym False))
