...
-- output_type: {Int,Int}
-- param_list: h xs tmp2 t
data Expr1_0
  = Param21_0
  | Prod1_0 (UnionM Expr1_1) (UnionM Expr1_1)
  | Incre1_0 (UnionM Expr1_1)
  deriving stock (Generic, Show)
...
instance GenSym (Int) Expr0_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    ...
    genSingle1 = [mrgCzero0_1] ++ [mrgDnum0_1]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        res <- chooseUnionFresh (genSingle0 ++ [mrgProd0_0 e1_0 e1_1] ++ [mrgIncre0_0 e1_2])
        return res
    gen1 gendepth
...
instance GenSym (Int) Expr1_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    ...
    genSingle1 = [mrgParam01_1] ++ [mrgCzero1_1] ++ [mrgDnum1_1]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        e1_1 <- (gen1 (gendepth - 1))
        e1_2 <- (gen1 (gendepth - 1))
        res <- chooseUnionFresh (genSingle0 ++ [mrgProd1_0 e1_0 e1_1] ++ [mrgIncre1_0 e1_2])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
...
instance GenSym (Int) Expr2_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgCzero2_0] ++ [mrgDnum2_0]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        e1_1 <- (gen1 (gendepth - 1))
        e2_0 <- (gen2 (gendepth - 1))
        res <- chooseUnionFresh (genSingle0 ++ [mrgCadd2_0 e0_0 e0_1] ++ [mrgCsub2_0 e0_2 e0_3] ++ [mrgCIte2_0 e2_0 e0_4 e0_5] ++ [mrgAccess0_1_2_0 e1_0] ++ [mrgAccess1_1_2_0 e1_1])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
        ...
        e0_1 <- (gen0 (gendepth - 1))
        e0_2 <- (gen0 (gendepth - 1))
        res <- chooseUnionFresh (genSingle1 ++ [mrgProd2_1 e0_0 e0_1] ++ [mrgIncre2_1 e0_2])
        return res
    gen2 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle2
      | otherwise = do
...
eval0_0 env (Incre0_0 p0) = incre (evalU0_1 env p0)
...
eval1_0 env (Incre1_0 p0) = incre (evalU1_1 env p0)
...
double n = 
        ...
        f xs = 
          case xs of
            ...
            Cons h t -> 
              ...
              let
                info =
                  (incre (h +  (h +  (fst subres))))
              in
              ((fst info), (Cons (snd info) (snd subres)))
  ...
  let
    info =
      (current (snd n))
  in
  (((fst n) +  ((fst n) +  (fst info))), (snd info))

f :: SymInteger -> SymInteger
f n = 
      mrgIte (n ==~  0)
        (1)
        (2 *  (f (n -  1)))
