        ...
        Cons h t -> 
          ...
          let
            x =
              (fst res)
          in
          mrgIte (evalU0_0 (RefEnv [("h", (Env0 h)), ("x", (Env0 x)), ("ms", (Env0 ms)), ("res", (Env1 res))]) ((genSym (5::Int) "hole0") :: (UnionM Expr0_0)))
            (((evalU1_0 (RefEnv [("h", (Env0 h)), ("x", (Env0 x)), ("ms", (Env0 ms)), ("res", (Env1 res))]) ((genSym (5::Int) "hole1") :: (UnionM Expr1_0))), (max' ms (1 +  (evalU2_0 (RefEnv [("h", (Env0 h)), ("x", (Env0 x)), ("ms", (Env0 ms)), ("res", (Env1 res))]) ((genSym (1::Int) "hole2") :: (UnionM Expr2_0)))))))
            (mrgIte (b (Cons h (Nil Unit)))
              (((let
                tmp1 =
                  (Cons h (Nil Unit))
              in
              evalU3_0 (RefEnv [("x", (Env0 x)), ("ms", (Env0 ms)), ("h", (Env0 h)), ("res", (Env1 res))]) ((genSym (5::Int) "hole3") :: (UnionM Expr3_0))), (max' ms 1)))
              (((let
                tmp2 =
                  (Nil Unit)
              in
              evalU4_0 (RefEnv [("x", (Env0 x)), ("ms", (Env0 ms)), ("h", (Env0 h)), ("res", (Env1 res))]) ((genSym (5::Int) "hole4") :: (UnionM Expr4_0))), ms)))
        _ -> ((let
            tmp3 =
              (Nil Unit)
          in
...
minimum' :: List -> SymInteger
minimum' x = 
  fold (\h t -> min' h t) x inf

isval x = 
  case x of
    Cons h t -> mrgIte (h >~  (minimum' t))
        ((toSym False))
        ((toSym True))
    _ -> (toSym True)
