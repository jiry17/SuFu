  ...
  fresh gendepth = gen0 gendepth 
    ...
    gen1 gendepth
      ...
      | otherwise = do
        ...
        e1_6 <- (gen1 (gendepth - 1))
        e1_7 <- (gen1 (gendepth - 1))
        res <- chooseUnionFresh (genSingle1 ++ [mrgCadd1_1 e1_0 e1_1] ++ [mrgAccess01_1 e0_0] ++ [mrgAccess11_1 e0_1] ++ [mrgAccess21_1 e0_2] ++ [mrgDiv1_1 e1_6 e1_7])
        return res
    gen2 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle2
      | otherwise = do
  ...
  fresh gendepth = gen0 gendepth 
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        e1_2 <- (gen1 (gendepth - 1))
        e2_0 <- (gen2 (gendepth - 1))
        res <- chooseUnionFresh (genSingle0 ++ [mrgCadd2_0 e0_0 e0_1] ++ [mrgCsub2_0 e0_2 e0_3] ++ [mrgCIte2_0 e2_0 e0_4 e0_5] ++ [mrgAccess02_0 e1_0] ++ [mrgAccess12_0 e1_1] ++ [mrgAccess22_0 e1_2] ++ [mrgDiv2_0 e0_6 e0_7])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
...
single_pass v xs = 
    let run = (let f xs = 
                            case xs of
                                ...
                                Cons h t -> 
                                    let tmp1 = (f t) in 
                                        -- (div' (h + getThird tmp1) ((getSecond tmp1) + 1), (getSecond tmp1) + 1, h + getThird tmp1)
