...
instance GenSym (Int) Expr0_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam00_0] ++ [mrgParam10_0] ++ [mrgParam20_0] ++ [mrgParam40_0] ++ [mrgCzero0_0]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        res <- chooseUnionFresh (genSingle0 ++ [mrgCadd0_0 e0_0 e0_1] ++ [mrgCsub0_0 e0_2 e0_3] ++ [mrgCIte0_0 e1_0 e0_4 e0_5] ++ [mrgDiv20_0 e0_6] ++ [mrgCmul0_0 e0_7 e0_8])
        return res
    gen1 gendepth
...
instance GenSym (Int) Expr1_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam01_0] ++ [mrgParam11_0] ++ [mrgParam21_0] ++ [mrgParam31_0] ++ [mrgParam41_0] ++ [mrgCzero1_0]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        e0_8 <- (gen0 (gendepth - 1))
        e1_0 <- (gen1 (gendepth - 1))
        res <- chooseUnionFresh (genSingle0 ++ [mrgCadd1_0 e0_0 e0_1] ++ [mrgCsub1_0 e0_2 e0_3] ++ [mrgCIte1_0 e1_0 e0_4 e0_5] ++ [mrgDiv21_0 e0_6] ++ [mrgCmul1_0 e0_7 e0_8])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
...
instance GenSym (Int) Expr2_0 where 
  ...
  fresh gendepth = gen0 gendepth 
    where
    genSingle0 = [mrgParam02_0] ++ [mrgParam12_0] ++ [mrgParam22_0] ++ [mrgCzero2_0]
    ...
    gen0 gendepth
      ...
      | otherwise = do
        ...
        e0_8 <- (gen0 (gendepth - 1))
        e1_0 <- (gen1 (gendepth - 1))
        res <- chooseUnionFresh (genSingle0 ++ [mrgCadd2_0 e0_0 e0_1] ++ [mrgCsub2_0 e0_2 e0_3] ++ [mrgCIte2_0 e1_0 e0_4 e0_5] ++ [mrgDiv22_0 e0_6] ++ [mrgCmul2_0 e0_7 e0_8])
        return res
    gen1 gendepth
      | gendepth <= 0 = chooseUnionFresh genSingle1
      | otherwise = do
...
eval0_0 env (Param10_0) = evalVar0 env "b"
eval0_0 env (Param20_0) = evalVar0 env "now"
eval0_0 env (Param40_0) = evalVar0 env "r"
...
eval0_0 env (Div20_0 p0) = div2 (evalU0_0 env p0)
...
eval1_0 env (Param11_0) = evalVar0 env "b"
eval1_0 env (Param21_0) = evalVar0 env "r"
eval1_0 env (Param31_0) = evalVar0 env "tmp2"
eval1_0 env (Param41_0) = evalVar0 env "now"
...
eval1_0 env (Div21_0 p0) = div2 (evalU1_0 env p0)
...
eval2_0 env (Param02_0) = evalVar0 env "b"
...
eval2_0 env (Div22_0 p0) = div2 (evalU2_0 env p0)
      ...
      mrgIte (now >~  r)
        (let
          tmp1 =
            (Nil Unit)
        in
        evalU0_0 (RefEnv [("a", (Env0 a)), ("b", (Env0 b)), ("now", (Env0 now)), ("r", (Env0 r))]) ((genSym (1::Int) "hole0") :: (UnionM Expr0_0)))
        (let
          tmp2 =
            (f (now +  1) r)
        in
        evalU1_0 (RefEnv [("a", (Env0 a)), ("b", (Env0 b)), ("r", (Env0 r)), ("tmp2", (Env0 tmp2)), ("now", (Env0 now))]) ((genSym (1::Int) "hole1") :: (UnionM Expr1_0)))
  in
  f  a b
  ...
  mrgIte (a <=~  b)
    (let
      tmp3 =
        (from' a b)
    in
    evalU2_0 (RefEnv [("b", (Env0 b)), ("a", (Env0 a)), ("tmp3", (Env0 tmp3))]) ((genSym (1::Int) "hole2") :: (UnionM Expr2_0)))
    (0)
...
ioPair :: [((SymInteger, SymInteger), Integer)] -> IO ()
ioPair pairs = do
    ...
    case res of
        ...
        Right model -> do
            print "success!"
    where
        constraint :: [((SymInteger, SymInteger), Integer)] -> SymBool
        constraint [] = con True
