...
type cartTree = Node of cartTree * int * cartTree | Leaf of unit
type cartPath = ConsNode of cartTree * cartPath | NilNode of unit
...
let l2cart order =
  let insert w =
    let rec insert tmp p =
      ...
      | _ -> ConsNode (Node (tmp, w, Leaf ()), NilNode ())
    in
    insert (Leaf ())
  in
  let merge =
...
let isval l = (maximum l) < (length l)
